...
<aui:form method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "saveEntry();" %>'>
	...
	<aui:fieldset>
        ...
        <%-- This is the changed part. The type is not choosable but depends on the currenet friendly url--%>
        <%-- The curly brackets may be important if this page is shown more than once on a page --%>
        <%
            {String selType = themeDisplay.getLayout().getFriendlyURL().replace("/", "");
        %>
           <aui:input type="hidden" name="type" value="<%= selType%>" />
        <%}%>
        <%-- The changed part ends here. --%>
